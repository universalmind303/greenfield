...
describe('Static File Server', () => {
  ...
  xit( 'should serve files from the `public` directory by default', (done) => {
    request('http://127.0.0.1:' + 8080, function(err, resp, body) {
      fs.readFileAsync('./public/index.html', 'utf8').then(file=>{
        expect(body).to.deep.equal(file)        
      })
      done();
    });
  });
  it( 'should serve files from the `process.env.PUBLIC` if specified', (done) => {
